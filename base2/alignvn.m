...
global glv
    ...
    Xkpk = zeros(length(imu)/nn,30);    % 预设状态估计及其均方误差存储空间 
      
    %%  卡尔曼滤波初始化
    % 状态估计初始值和均方误差初始值给定
    t = 100;
    Xk = zeros(15, 1);                  % 状态量Xk=[φ; δV; εb; ▽;Psy ]  
    Pk = diag([[ 1; 1; 1]*glv.deg; [1; 1; 1]; imuerr.eb; imuerr.db; 435; 435; 435])^2;   % 系统速度误差激励噪声序列
    Qk = diag([imuerr.web; imuerr.wdb; zeros(6,1); 200; 200; 200])^2*nts;             % 系统噪声方差阵
    Rk = diag([1; 1; 1; 200; 200; 200])^2;   % 量测噪声方差阵
    Ft = zeros(15);                     % 连续型系统方程F阵初始化  
