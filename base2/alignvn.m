...
global glv
    ...
    Xkpk = zeros(length(imu)/nn,26);    % 预设状态估计及其均方误差存储空间 
      
    %%  卡尔曼滤波初始化
    % 状态估计初始值和均方误差初始值给定
    Xk = zeros(13, 1);                  % 状态量Xk=[φ; δV; εb; ▽;Psy ]  
    Pk = diag([[1; 1; 1]*glv.deg; [1 ; 1; 1]; imuerr.eb; imuerr.db; 3*glv.deg])^2;   % 系统速度误差激励噪声序列
    Qk = diag([imuerr.web; imuerr.wdb; zeros(6,1); 0.0001])^2*nts;             % 系统噪声方差阵
    Rk = diag([1; 1; 1; 0.5])^2;   % 量测噪声方差阵
    Ft = zeros(13);                     % 连续型系统方程F阵初始化  
    H1 = [1;0;0;0];  H2 = [0;1;0;0];   H3 = [0;0;1;0];   H4 = [0;0;0;1];
    Hk = [zeros(4,3),H1,H2,H3,zeros(4,6),H4];  % 量测阵
