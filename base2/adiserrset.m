function imuerr = adiserrset( sqrtR0G, TauG, sqrtR0A, TauA, dKGii, dKAii, dKGij, dKAij, KA2)
% 功能：设置ADIS16475IMU的误差
% 输入： sqrtR0G,TauG
%        sqrtR0A,TauA
...
global glv
    ...
    %% ADIS16475的固定偏差&随机游走
    % eb-陀螺恒定漂移偏差（deg/h），db-加速度计常数偏差（ug）
    % web-陀螺角随机游走系数（deg/sqrt(h)），wdb-加速度计随机游走系数（ug/sqrt(h)）
    imuerr.eb(1:3) = [2;2.50;7]*glv.dph;   imuerr.web(1:3) = [0.15;0.15;0.3]*glv.dpsh;
    imuerr.db(1:3) = 3.6*glv.ug;    imuerr.wdb(1:3) = 0.012*glv.mpspsh;
